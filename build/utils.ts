...
export const copy = (src: string, dst: string): Promise<void> => new Promise((resolve, reject) => ncp(src, dst, (err: any) => (err ? reject(err) : resolve())))

export const exists = (path: string): Promise<boolean> => new Promise((resolve, _) => ex(path, exists => resolve(exists)))

export const create = (path: string): Promise<void> => new Promise((resolve, reject) => exists(path).then(exists => (!exists ? mkdir(path, err => (err ? reject(err) : resolve())) : resolve())))

export const run = (cmd: string, cwd?: string): Promise<string> => new Promise((resolve, reject) => exec(cmd, { cwd }, (err, out) => (err ? reject(err) : resolve(out))))

export const remove = (path: string): Promise<string> => run('rm -fr ' + path)
